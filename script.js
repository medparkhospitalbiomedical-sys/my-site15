...
async function startScanner(cameraIndex = 0) {
    if (!camerasList[cameraIndex]) {
        alert('ไม่พบกล้องที่สามารถใช้งานได้');
        return;
    }

    const cameraId = camerasList[cameraIndex].id;
...
scanQrBtn.addEventListener('click', async () => {
    if (qrReaderContainer.style.display === 'none') {
        ...
        try {
            ...
            // จัดเรียงกล้อง: กล้องหลังเป็นอันดับ 1
            camerasList = cameras.sort((a, b) => {
            ...
            currentCameraIndex = 0; // เริ่มที่กล้องหลังอันดับ 1
...
switchCameraBtn.addEventListener('click', async () => {
    if (!html5QrCodeScanner || camerasList.length <= 1) return;

    await html5QrCodeScanner.stop(); // หยุดกล้องปัจจุบัน
    currentCameraIndex = (currentCameraIndex + 1) % camerasList.length; // สลับไปกล้องถัดไป
    startScanner(currentCameraIndex);
});
